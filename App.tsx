...
const App: React.FC = () => {
  ...
  const startNewSession = () => {
    ...
    // START PROTOCOL: Generate Fragment 1 immediately
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-8 bg-[url('https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&q=80&w=2069')] bg-cover bg-center relative">
        ...
        {user && (
          <div className="absolute top-6 right-6 z-20 flex items-center gap-4 bg-black/40 p-2 rounded-full border border-cyan-500/20 backdrop-blur-md">
            <span className="text-[10px] text-cyan-400 cyber-text uppercase tracking-widest pl-4">LINKED: {user.email?.split('@')[0]}</span>
            <button onClick={handleSignOut} className="px-4 py-1 bg-red-500/10 text-red-500 border border-red-500/40 rounded-full text-[10px] cyber-text uppercase hover:bg-red-500 hover:text-black transition-all">Sever Connection</button>
          </div>
        )}
        ...
        <div className="relative z-10 space-y-8 animate-in fade-in duration-1000">
          ...
          <p className="text-cyan-400 text-lg md:text-xl cyber-text max-w-lg mx-auto leading-relaxed border-l-2 border-cyan-500 pl-4">MISTRAL-LARGE NARRATIVE ACTIVE.<br/>EDGE-UNIVERSAL SPEECH SYNTHESIZED.</p>
          
          <div className="flex flex-col gap-4 max-w-sm mx-auto w-full">
            <button onClick={() => setView('setup')} className="bg-magenta-600 hover:bg-magenta-500 text-white font-bold py-4 px-10 rounded-sm cyber-text border-b-4 border-magenta-900 shadow-[0_0_30px_rgba(255,0,255,0.4)] transition-all uppercase">Enter Neon City</button>
            <button onClick={() => { refreshSessions(user?.id); setView('dashboard'); }} className="bg-cyan-900/40 hover:bg-cyan-800/60 text-cyan-400 font-bold py-3 px-10 rounded-sm cyber-text border border-cyan-500 transition-all uppercase">Neural Archives</button>
    ...
    return (
      <div className="min-h-screen p-6 max-w-xl mx-auto space-y-10 pt-16 animate-in slide-in-from-bottom duration-500">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] uppercase text-cyan-500 font-bold tracking-[0.2em]">Plot Injector (Optional)</label>
            <textarea value={customPlot} onChange={(e) => setCustomPlot(e.target.value)} placeholder="Narrative parameters..." className="w-full bg-black border border-cyan-500/30 p-4 rounded text-white focus:border-cyan-500 outline-none h-24" />
          </div>
        ...
        <div className="flex gap-4">
          <button onClick={() => setView('landing')} className="flex-1 py-4 border border-red-500 text-red-500 cyber-text uppercase text-sm font-bold">Abort</button>
     ...
     return (
        <div className="min-h-screen p-6 max-w-3xl mx-auto pt-16">
           <div className="flex justify-between items-end mb-10 border-b border-magenta-500/30 pb-4">
              <h2 className="text-4xl font-bold cyber-text text-white uppercase glow-magenta">Neural Logs</h2>
           ...
           <div className="grid gap-4">
              {savedSessions.length === 0 ? (
                <div className="text-center py-20 text-gray-600 border border-dashed border-gray-800">NO LOGS FOUND</div>
              ) : savedSessions.map(s => (
                 <div key={s.id} className="bg-cyan-900/10 border border-cyan-500/30 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group">
                    <div>
                       <h3 className="text-xl font-bold cyber-text text-white">{s.genre}</h3>
                       <p className="text-[10px] text-gray-500 uppercase">{new Date(s.startTime).toLocaleDateString()} // {s.chapters.length} segments</p>
                    </div>
                    <div className="flex gap-2 w-full md:w-auto">
                      <button onClick={() => { setCurrentSession(s); setView('active'); }} className="flex-1 md:flex-none px-6 py-2 bg-magenta-600 text-white cyber-text text-xs font-bold uppercase">Resume Link</button>
